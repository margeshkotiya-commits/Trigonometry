...
export default function TutorialOverlay({ step, setStep }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none p-4">
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full relative z-10 pointer-events-auto border border-slate-100 transform transition-all">
        <button 
          onClick={() => setStep(0)}
        ...
        <div className="mb-2 text-indigo-600 font-bold text-sm uppercase tracking-wider">
          Step {step} of {TUTORIAL_STEPS.length}
        </div>
        
        <h3 className="text-xl font-bold text-slate-800 mb-3">{currentStep.title}</h3>
        ...
        <div className="flex justify-between items-center">
          <div className="flex gap-1.5">
            {TUTORIAL_STEPS.map((_, i) => (
              <div key={i} className={`h-2 rounded-full transition-all ${i + 1 === step ? 'w-6 bg-indigo-600' : 'w-2 bg-slate-200'}`} />
            ))}
          </div>
          
          <button 
            onClick={() => setStep(isLast ? 0 : step + 1)}
            className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-medium hover:bg-indigo-700 transition-colors shadow-sm shadow-indigo-200"
          >
            {isLast ? (
